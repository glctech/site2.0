...
(there are three *dead* legacy attempts still in the repo — see
[Legacy files](#legacy-files-ignore-these)).
...
- [Legacy files (ignore these)](#legacy-files-ignore-these)
...
These files are **not loaded by any page** and use a different, incompatible
key scheme. They're historical dead code. Do not edit them expecting a change,
and don't wire them back in without consolidating first.

| File | Old scheme | Status |
|------|-----------|--------|
| `js/i18n.js` | fetches `/lang/{lang}.json`, keys like `nav_about` | **dead** — not referenced |
| `lang.js` | nested object `translations.pt.nav.home` | **dead** — not referenced |
| `lang/en.json`, `lang/pt.json` | flat JSON, keys like `nav_about` | **dead** — used only by `js/i18n.js` |
...
into JSON, that's a real refactor — do it deliberately, remove the legacy
files, and update every page's `<script>` tag.
